...
function scr_player_ghost()
{
	...
	if scr_isnoise()
	{
		...
		else
		{
			...
			if (scr_solid_player((x + sign(hsp)), (y + sign(vsp))) && (!(place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_destructibles))) && (ghostpepper < 3 || (!(place_meeting((x + sign(hsp)), (y + sign(vsp)), obj_metalblock)))) && (!(check_slope((x + sign(hsp)), (y + sign(vsp)), obj_slope))))
			{
				ghostbumpbuffer++
	...
	if sprite_index != spr_ghostend
	{
		...
		else
		{
			...
			if m != 0
			{
				...
				if (scr_ispeppino() && scr_solid_player((x + sign(hsp)), y) && (!(check_slope((x + sign(hsp)), (y + sign(vsp)), obj_slope))))
					movespeed = 0
				if ghostbump < 0
				{
					hsp = (dx * ghostbump)
