...
{
	var slope = scr_slope() && (global.leveltosave != "etb" or key_down);
	...
	if ((sprite_index == spr_knightpepdowntrust || sprite_index == spr_knightpepfall || sprite_index == spr_knightpepdoublejump || sprite_index == spr_playerN_knightdoublejumpfall) && grounded && vsp > 0 && !slope)
	{
		with (obj_baddie)
		{
	...
	if (move != 0 && hsp != 0 && sprite_index != spr_knightpepdoublejump)
	{
		...
		image_speed = 0.35;
	if (floor(image_index) == 4 && sprite_index == spr_knightpepstart && (!instance_exists(obj_thunder) or !REMIX))
