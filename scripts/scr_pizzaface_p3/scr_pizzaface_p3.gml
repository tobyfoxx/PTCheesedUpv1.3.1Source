...
function scr_pizzaface_p3_update_sounds()
{
	...
	sound_instance_move(snd_bigpunch, x, y);
	if (sprite_index == spr_pizzahead_swinging)
	{
		...
		sound_instance_move(snd_swinging, x, y);
	}
	else
...
function scr_pizzaface_p3_walk()
{
	...
	else
	{
		...
		{
			hsp = Approach(hsp, 0, 0.5);
			create_particle(x, y + 33, part.landcloud);
			vsp = -5;
			touchedground = true;
		}
	}
...
function scr_pizzaface_p3_stomp()
{
	...
	if (floor(image_index) >= 20 && !shot)
	{
		...
		create_particle(x + (119 * image_xscale), y + 35, part.groundpoundeffect);
		shake_camera(6, 5 / room_speed);
		with (obj_player)
		{
			if (place_meeting(x, y, other))
			{
			...
			switch (substate)
			{
				...
				case states.finishingblow:
					if (floor(image_index) >= 3 && !shot)
					{
						...
						{
							linethrown = true;
							scr_hitstun_enemy(id, 1, -image_xscale * 25, -5);
						}
					}
					break;
			}
