...
function scr_playerN_machcancel()
{
	...
	if (input_buffer_slap > 0 && key_up && (!global.pistol || scr_isnoise()))
	{
		input_buffer_slap = 0;
		state = states.punch;
		image_index = 0;
	...
	if (scr_isnoise() && key_up && input_buffer_jump > 0 && !scr_check_groundpound2())
	{
		freefallstart = 0;
		railmomentum = false;
...
function scr_noise_machcancel_grab()
{
	...
	else if key_shoot2
	{
		...
		if move != 0
			xscale = move;
		scr_perform_move(moves.shootattack, state);
