...
{
	if live_call(text, pos, return_array) return live_result;
	...
	var offset_x = info.x, offset_y = info.y;
...
function scr_draw_granny_texture(x, y, xscale, yscale, tex_x, tex_y, sprite = spr_pizzagrannytexture, bubble_sprite = spr_tutorialbubble, txo = 0, tyo = 0)
{
...
{
	if live_call(x, y, text_arr, color, alpha, effect, option_struct) return live_result;
	...
	for (var i = 0; i < array_length(text_arr); i++)
	{
		...
		switch t
		{
			case texttype.icon:
				...
				switch val
				{
					...
					case textkey.taunt:
						...
						break;
					case textkey.shoot:
						icon = tdp_get_tutorial_icon("player_shoot");
						break;
			...
			case texttype.array:
				var val2 = b[4];
				var f = font_get_offset();
				scr_draw_text_arr(cx + f.x, cy + f.y, val2, color, alpha, val);
				break;
