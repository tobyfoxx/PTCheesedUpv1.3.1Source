...
function scr_vigilante_update_sounds()
{
	if sprite_index == spr_vigilante_flamethrower
	{
		...
		sound_instance_move(snd_flame, x, y);
	}
	else
	...
	if state == states.mach2 && sprite_index != spr_playerV_crouchmove
	{
		...
		sound_instance_move(snd_mach1, x, y);
	}
	else
		fmod_event_instance_stop(snd_mach1, false);
	sound_instance_move(snd_slide, x, y);
	if sprite_index == spr_vigilante_uziprepare
	{
		...
		sound_instance_move(snd_uziprep, x, y);
	}
	else
	...
	if state == states.duel
		...
		songstate = Approach(songstate, check_char("V") ? 2 : 0, 0.05);
	with obj_music
	{
		if music != -4
...
{
	var skip_pistol = check_char(["N", "V"]);
	if skipintro && !introwait
	{
		...
		sprite_index = skip_pistol ? spr_playerV_idle : spr_playerV_dynamitethrow;
		image_index = 0;
		introwait = true;
		introbuffer = 250;
	...
	if !introwait
	{
		...
		with obj_player1
		{
			...
			flash = false;
			x = roomstartx;
			if (other.sprite_index == other.spr_intro1)
			{
				if character == "V"
					sprite_index = spr_vigilante_intro1snotty;
			...
			{
				if (sprite_index == spr_gnomecutscene2 || sprite_index == spr_playerN_bosscutscene2 || sprite_index == spr_playerN_bosscutscene3)
					image_index = image_number - 1;
				else if sprite_index == spr_vigilante_intro2snotty
					image_index = 3;
				else if sprite_index == spr_vigilante_intro3snotty
					sprite_index = spr_vigilante_intro4snotty;
		...
		{
			introbuffer--;
			if (floor(image_index) == image_number - 1 && sprite_index == spr_intro2)
				image_index = image_number - 1;
			if (sprite_index == spr_intro2 && floor(image_index) == 13)
			{
				...
				with obj_player1
				{
					if character == "V"
					{
						sprite_index = spr_vigilante_intro3snotty;
						image_index = 0;
					}
					else if scr_ispeppino()
					{
						if sprite_index == spr_gnomecutscene2
						{
							...
							image_index = 0;
							sound_play(global.snd_screamboss);
						}
					}
					else
					{
		...
		else if (sprite_index == spr_intro1)
		{
			...
			with obj_player1
			{
				image_index = 0;
				if character == "V"
					sprite_index = spr_vigilante_intro2snotty;
				else if scr_ispeppino()
		...
		else if (sprite_index == spr_intro2)
		{
			if !skip_pistol
			{
				with (instance_create(x, y, obj_pistolpickup))
	...
	else
	{
		...
		else if sprite_index != spr_vigilante_impatient
		{
			...
			image_index = image_number - 1;
		if global.pistol or skip_pistol
		{
			sprite_index = spr_playerV_idle;
			state = states.walk;
		}
...
function scr_vigilante_duel()
{
	...
	var _cheat = !scr_ispeppino(obj_player1) || global.swapmode;
	if duelphase == 2
	{
		with obj_player1
		{
	...
	else if duelphase < 2
	{
		with obj_player1
		{
			if key_slap2 && scr_ispeppino() && !_cheat
			{
				other.duelbuffer = 0;
				other.duelphase = 2;
	...
	if duelbuffer > 0
	{
		...
		switch duelphase
		{
			...
			case 1:
				...
				if (scr_ispeppino(obj_player1) && signy <= -sprite_get_height(signspr))
				{
					duelbuffer = 80;
		...
		if duelphase == 2 && scr_ispeppino(obj_player1) && !_cheat
		...
		switch duelphase
		{
			...
			case 2:
				...
				else
				{
					...
					if check_char("V")
					{
						layer_background_change(bg2, bg_snottyboss)
						layer_background_change(bg1, bg_snottyarena)
						layer_background_change(layer_background_get_id("Backgrounds_scroll1"), bg_snottybg_pre)
					}
					else
					{
					...
					with obj_player1
					{
						...
						sprite_index = spr_duelend;
						image_index = character == "P" ? 2 : 0;
						state = states.actor;
						actorbuffer = 120;
					}
				}
