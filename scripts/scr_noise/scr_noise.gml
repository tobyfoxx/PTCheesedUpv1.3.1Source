...
function scr_noise_update_sounds()
{
	...
	if state == states.mach2 && grounded && vsp > 0
	{
		...
		sound_instance_move(snd_skateloop, x, y);
	}
	else
	...
	if state == states.bounce
	{
		...
		sound_instance_move(snd_jetpackspin, x, y);
	}
	else
	...
	if state == states.drop
	{
		...
		sound_instance_move(snd_balloon, x, y);
	}
	else
	...
	if state == states.droptrap
	{
		...
		sound_instance_move(snd_droptrap, x, y);
	}
	else
	...
	if sprite_index == spr_playerN_fightball
	{
		...
		else
		{
			...
			sound_play_3d("event:/sfx/noise/fightball", x, y);
		}
	}
	else
		fightball_snd_buffer = 0;
...
function scr_noise_arenaintro()
{
	...
	// PEPPINO meets NOISE
	if scr_ispeppino(obj_player1) && !doise
	{
		if !skipintro
		{
			if !intro
...
function scr_noise_mach2()
{
	...
	if !golf
	{
		...
		{
			sound_play_3d("event:/sfx/noise/machslide", x, y);
	...
	{
		sound_play_3d("event:/sfx/fakepep/jump", x, y);
		vsp = -11;
		attackspeed = 8;
		image_index = 0;
	...
	if ix != image_xscale && grounded && sprite_index == spr_playerN_mach && state != states.machslide
	{
		if !golf
		{
			sound_play_3d("event:/sfx/noise/skateturn", x, y);
			state = states.machslide;
...
function scr_noise_spin()
{
	...
	if (floor(image_index) == image_number - 4 && sprite_index == spr_playerN_spin)
	{
		...
		{
			sound_play_3d("event:/sfx/noise/pogo", x, y);
			state = states.pogo;
...
function scr_noise_pogo()
{
	...
	if grounded && vsp > 0 && bounced
	{
		...
		else
		{
			...
			if phase == 2
			{
				sound_play_3d("event:/sfx/noise/noisecrusher", x, y);
...
function scr_noise_dropstart()
{
	...
	{
		create_particle(x + irandom_range(-12, 12), y + 43, part.cloudeffect);
		cloudbuffer = 5;
	}
	if cloudbuffer > 0
		cloudbuffer--;
	...
	switch substate
	{
		...
		case states.bump:
			...
			if (instance_exists(obj_noisebosscrate) && obj_noisebosscrate.y >= y - 10)
			{
				...
				else
				{
					...
					image_index = 0;
					substate = states.shotgun;
					sound_play_3d("event:/sfx/pep/groundpound", obj_player1.x, obj_player1.y);
					sound_play_3d("event:/sfx/voice/noisepositive", x, y);
		...
		case states.ending:
			if !doise
			{
				...
				{
					sound_play("event:/sfx/voice/noisescream");
