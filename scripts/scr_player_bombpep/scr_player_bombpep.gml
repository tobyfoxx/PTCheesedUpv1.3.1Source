function scr_player_bombgrab()
{
	...
	if (sprite_index != spr_haulingstart && sprite_index != spr_uppercutfinishingblow && sprite_index != spr_throw)
	{
		...
		if (sprite_index == spr_haulingjump && floor(image_index) == (image_number - 1))
			sprite_index = spr_haulingfall;
		if scr_slapbuffercheck() && (scr_ispeppino() or instance_exists(obj_cyop_loader))
		{
			scr_resetslapbuffer();
...
function scr_player_bombpepside()
{
	...
	with (instance_place(x + hsp, y, obj_metalblock))
		instance_destroy();
	if (scr_solid(x + xscale, y) && (scr_solid_slope(x + sign(hsp), y) || check_solid(x + sign(hsp), y)) && !place_meeting(x + sign(hsp), y, obj_metalblock) && !place_meeting(x + sign(hsp), y, obj_destructibles))
	{
...
function scr_player_bombpep()
{
	...
	if (bombpeptimer > 0)
		bombpeptimer -= 0.5;
	if (scr_solid(x + 1, y) && xscale == 1 && hsp != 0 && (!check_slope(x + sign(hsp), y) || scr_solid_slope(x + sign(hsp), y)))
	{
		instance_create(x + 10, y + 10, obj_bumpeffect);
		xscale *= -1;
