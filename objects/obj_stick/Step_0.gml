...
else
{
	...
	sound_instance_move(global.snd_mrstickhat, x, y);
}
switch state
{
	case states.fall:
	...
	case states.normal:
		...
		if (drawing && obj_player1.key_up && (global.pigtotal - global.pigreduction) >= maxscore)
		{
			...
			if noisette
			{
				...
				buffer = 80;
				wet_buffer = 0;
				sound_play_3d("event:/sfx/noisette/voice1", x, y);
			}
			gamesave_async_save();
			with obj_player
			{
		...
		if sprite_index == spr_noisettestick_takemoney1
		{
			if obj_player1.hsp == 0
			{
				...
				else
				{
					...
					{
						sound_play_3d("event:/sfx/noisette/voice2", x, y);
		...
		else if sprite_index == spr_noisettestick_takemoney2
		{
			...
			if (abs(x - tx) <= 15)
			{
				...
				if !shot
				{
					...
					sound_instance_move(kiss_snd, x, y);
					with obj_player1
						sprite_index = spr_bump;
					shot = true;
					buffer = 150;
