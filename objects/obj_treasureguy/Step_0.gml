...
switch (state)
{
	...
	case states.underground:
		...
		if (underground)
		{
			...
			visible = false;
			if (player.x > (x - 80) && player.x < (x + 80) && y <= (player.y + 60) && y >= (player.y - 60)
			&& (player.state == states.freefallland || player.sprite_index == player.spr_piledriverland
			|| (player.state == states.ratmountbounce && player.vsp > 0)))
			{
